...
export function normalizeAdminRoles(
  value: unknown,
  legacyRole?: unknown,
): AdminRole[] {
  ...
  if (
    ...
    ADMIN_ROLES.includes(legacyRole as AdminRole)
  )
    roles.push(legacyRole as AdminRole);
...
export function canViewAdminIdentity(
  viewerUid: string,
  subjectUid: string,
  subjectRoles: AdminRoles,
) {
  return viewerUid === subjectUid || !subjectRoles.includes("super_admin");
}
