...
describe("public validation", () => {
  ...
  it("limits sensitive member IDs", () =>
    expect(
      insuranceVerificationSchema.safeParse({
        fullName: "Jane Doe",
        phone: "024 000 0000",
        insurer: "Test",
        memberId: "x".repeat(81),
        serviceNeeded: "Consultation",
      }).success,
    ).toBe(false));
  it("requires telemedicine consent", () =>
    expect(
      telemedicineRequestSchema.safeParse({
        fullName: "Jane Doe",
        phone: "024 000 0000",
...
describe("patient feedback", () => {
  ...
  it("rejects a future visit date", () =>
    expect(
      feedbackSchema.safeParse({ ...valid, visitDate: "2999-01-01" }).success,
    ).toBe(false));
  it("rejects unknown fields", () =>
    expect(
      feedbackSchema.safeParse({ ...valid, patientId: "secret" }).success,
    ).toBe(false));
  it("requires dissatisfaction context", () =>
    expect(
      feedbackSchema.safeParse({
        ...valid,
        overallSatisfaction: "very_dissatisfied",
      }).success,
    ).toBe(false));
  it("requires receipt details for a concern", () =>
    expect(
      feedbackSchema.safeParse({ ...valid, receiptConcern: "yes" }).success,
    ).toBe(false));
  it("requires contact channel when follow-up is requested", () =>
    expect(
      feedbackSchema.safeParse({ ...valid, contactRequested: true }).success,
    ).toBe(false));
  it("accepts non-identifying campaign attribution", () =>
    expect(
      feedbackSchema.safeParse({
        ...valid,
        campaign: "screening_aug_2026",
        source: "sms",
      }).success,
    ).toBe(true));
});
...
describe("SMS safety", () => {
  it("removes invalid and duplicate recipients", () =>
    expect(
      deduplicateRecipients(["0241234567", "+233241234567", "invalid"]),
    ).toEqual(["+233241234567"]));
  it("uses a non-delivering mock provider", async () => {
        ...
        await provider.sendBatch([
          {
            to: "+233241234567",
            message: "Test",
            idempotencyKey: "campaign-recipient",
          },
        ])
      )[0],
    ).toEqual({ providerId: "mock-campaign-recipient", status: "mocked" });
  });
});

describe("security utilities", () => {
  it("generates non-document references", () =>
    expect(createReference("APT")).toMatch(/^SGH-APT-[A-F0-9]{12}$/));
  ...
  it("hides super administrator identities from every other administrator", () => {
    expect(canViewAdminIdentity("admin-1", "owner-1", ["super_admin"])).toBe(
      false,
    );
    expect(canViewAdminIdentity("super-2", "owner-1", ["super_admin"])).toBe(
      false,
    );
    expect(canViewAdminIdentity("owner-1", "owner-1", ["super_admin"])).toBe(
      true,
    );
    expect(canViewAdminIdentity("admin-1", "admin-2", ["admin"])).toBe(true);
  });
  it("combines multiple roles while preserving legacy role claims", () => {
    expect(
      normalizeAdminRoles(["insurance", "reception", "invalid"], "corporate"),
    ).toEqual(["reception", "insurance", "corporate"]);
    expect(hasPermission(["insurance", "content_editor"], "insurance")).toBe(
      true,
    );
    expect(hasPermission(["insurance", "content_editor"], "content")).toBe(
      true,
    );
    expect(hasPermission(["insurance", "content_editor"], "appointments")).toBe(
      false,
    );
  ...
  it("enforces safe submission transitions", () => {
