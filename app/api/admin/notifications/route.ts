...
function allowed(role: Parameters<typeof hasPermission>[0], type: string): type is NotificationType {
  return notificationTypes.includes(type as NotificationType) && hasPermission(role, submissionKinds[type as NotificationType].permission);
}
...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, unread: items.filter((item) => !item.read).length, items }, { headers: { "Cache-Control": "private, no-store" } });
}
...
export async function POST(request: Request) {
  ...
  targets.forEach((doc) => batch.set(adminDb.collection("admin_notification_states").doc(actor.uid).collection("items").doc(doc.id), { readAt: FieldValue.serverTimestamp() }, { merge: true }));
  await batch.commit();
