...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (data.status === "sending" && leaseUntil instanceof Date && leaseUntil.getTime() <= Date.now()) {
    ...
    if (queued === 0) {
      ...
      for (let offset = 0; offset < ambiguous.length; offset += 450) { const batch = adminDb.batch(); ambiguous.slice(offset, offset + 450).forEach((doc) => batch.update(doc.ref, { status: "failed", errorClass: "interrupted_delivery_unknown", updatedAt: FieldValue.serverTimestamp() })); await batch.commit(); }
      const failed = recipients.docs.filter((doc) => doc.data().status === "failed").length + ambiguous.length;
      const sent = recipients.docs.filter((doc) => ["sent", "accepted", "mocked", "delivered"].includes(String(doc.data().status))).length;
      const finalStatus = sent === 0 && failed ? "failed" : failed ? "partially_failed" : "completed";
      await snapshot.ref.update({ status: finalStatus, failedCount: failed, queuedCount: 0, processingLeaseUntil: null, updatedAt: FieldValue.serverTimestamp() });
      data = { ...data, status: finalStatus, failedCount: failed, queuedCount: 0, processingLeaseUntil: null };
    }
  }
