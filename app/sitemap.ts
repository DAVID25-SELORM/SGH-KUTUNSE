...
const staticRoutes = ["", "/about", "/services", "/doctors", "/appointments", "/insurance", "/telemedicine", "/screening", "/corporate-wellness", "/patient-resources", "/health", "/gallery", "/feedback", "/contact", "/directions", "/careers", "/privacy", "/terms"];

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = HOSPITAL.siteUrl;
  const now = new Date();
  const [doctors, articles, albums] = await Promise.all([getPublishedDoctors(), getPublishedArticles(), getPublishedGalleryAlbums()]);
  return [
    ...
    ...albums.map(album => ({ url: `${base}/gallery/${album.slug}`, lastModified: album.publishedAt ? new Date(album.publishedAt) : now })),
  ];
}
