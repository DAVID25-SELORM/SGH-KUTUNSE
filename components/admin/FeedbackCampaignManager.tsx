...
export function FeedbackCampaignManager({ initialCampaigns, canSend, providerMode, schedulingEnabled, initialSmsPolicy }: {
  ...
  const estimatedRecipients = source === "custom_list" ? parseRecipientImport(customContacts).recipients.length : (audienceSummary?.eligible ?? 0);
  const finalExclusions = (preparation?.noConsent ?? 0) + (preparation?.optedOut ?? 0) + (preparation?.invalidPhone ?? 0) + (preparation?.doNotContact ?? 0) + (preparation?.duplicates ?? 0) + (preparation?.otherExclusions ?? 0);
    ...
    <section className="rounded-2xl border bg-white p-5 sm:p-6">
      ...
      {!active ? <div className="mt-7">
        ...
        <div className="mt-7">
          ...
          <div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={testPhone} onChange={(event) => setTestPhone(event.target.value)} placeholder="Test phone number" className="min-h-11 rounded-xl border p-3 sm:w-80" /><button disabled={busy || !canSend || !testPhone.trim() || active.testSmsAccepted || ["sending", "delivery_unknown"].includes(active.testSendState ?? "") || hasUnsavedMessage} onClick={sendTest} className="min-h-11 rounded-xl border border-purple-deep px-5 py-3 font-semibold text-purple-deep disabled:opacity-50">{active.testVerified ? "Test Verified" : active.testSmsAccepted ? "Test SMS Sent" : active.testSendState === "sending" ? "Test SMS in progress" : active.testSendState === "delivery_unknown" ? "Delivery needs verification" : "Send Test SMS"}</button></div>
          {active.testSendState === "delivery_unknown" && <p className="mt-3 rounded-xl bg-amber-50 p-3 font-semibold text-amber-900">Do not send another test. The provider response was uncertain and the SMS may have arrived. Open the newest secure link on the handset and submit it to complete verification.</p>}
